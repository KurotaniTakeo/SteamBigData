...
df = df_raw.withColumn("price_num", when(col("price") == "Free", 0.0).otherwise(regexp_replace("price", "\\$", "").cast("double")))
...
def get_user_preferences():
    conn = pymysql.connect(host='localhost', user='root', password='123456', database='steam_games', charset='utf8')
...
def genre_score_expr(genres_array, user_genres):
    return reduce(lambda a, b: a + b, [when(array_contains(genres_array,g), 2).otherwise(0) for g in user_genres]) if user_genres else lit(0)

def category_score_expr(cat_array, user_categories):
    return reduce(lambda a, b: a + b, [when(array_contains(cat_array, c), 2).otherwise(0) for c in user_categories]) if user_categories else lit(0)
...
def generate_recommendations(refresh=False):
    ...
    for uid, prefs in user_prefs.items():
        ...
        df_scored = df_filtered.withColumn("genre_score", genre_score_expr(col("genres_array"), prefs["genres"])) \
                               ...
                               .withColumn("total_score", col("genre_score") + col("category_score") + col("platform_score") + col("price_score"))
        ...
        for u in similar_users:
            similar_df = df_filtered.withColumn("genre_score", genre_score_expr(col("genres_array"), user_prefs[u]["genres"])) \
                                    ...
                                    .withColumn("total_score", col("genre_score") + col("category_score") + col("platform_score") + col("price_score"))
            games = [r["appid"] for r in similar_df.orderBy(col("total_score").desc()).select("appid").limit(10).collect()]
